...
var BridgeProxy = class {
  ...
  getHostCapabilities() {
    var _a;
    return (_a = this.capabilities) != null ? _a : null;
  }
  getAccountContext() {
...
var CryptoOps = class {
  constructor(logger, performanceClient, skipValidateSubtleCrypto) {
    this.logger = logger;
    validateCryptoAvailable(skipValidateSubtleCrypto != null ? skipValidateSubtleCrypto : false);
...
async function initializeBaseRequest(request, config, performanceClient, logger) {
  ...
  if (config.cache.claimsBasedCachingEnabled && request.claims && !StringUtils.isEmptyObj(request.claims)) {
...
async function getAuthCodeRequestUrl(config, authority, request, logger, performanceClient) {
  ...
  RequestParameterBuilder_exports.addResponseTypeCode(parameters);
...
var PopupClient = class extends StandardInteractionClient {
  ...
  acquireToken(request, pkceCodes) {
    ...
    try {
      ...
      const popupParams = {
        ...
        popupWindowParent: (_a = request.popupWindowParent) != null ? _a : window
      };
  ...
  logout(logoutRequest) {
    ...
    try {
      ...
      const popupParams = {
        ...
        popupWindowParent: (_a = logoutRequest == null ? void 0 : logoutRequest.popupWindowParent) != null ? _a : window
      };
  ...
  async acquireTokenPopupAsync(request, popupParams, pkceCodes) {
    ...
    const pkce = pkceCodes || await invokeAsync(generatePkceCodes, PerformanceEvents.GeneratePkceCodes, this.logger, this.performanceClient, this.correlationId)(this.performanceClient, this.logger, this.correlationId);
...
var RedirectHandler = class {
  ...
  async handleCodeResponse(response, state) {
    ...
    let authCodeResponse;
    try {
      authCodeResponse = Authorize_exports.getAuthorizationCodePayload(response, requestState);
    } catch (e) {
      if (e instanceof ServerError && e.subError === userCancelled) {
        throw createBrowserAuthError(userCancelled);
      } else {
...
function checkIfRefreshTokenErrorCanBeResolvedSilently(refreshTokenError, cacheLookupPolicy) {
  const noInteractionRequired = !(refreshTokenError instanceof InteractionRequiredAuthError && refreshTokenError.subError !== InteractionRequiredAuthErrorCodes_exports.badToken);
...
function getPerfMeasurementModule() {
  ...
  try {
    ...
    if (Number(perfEnabled) === 1) {
      return import("./BrowserPerformanceMeasurement-OPLBJXTF.js");
    }
  } catch (e) {
  }
  return void 0;
